...
import { GitPullRequest, Bot, Loader2, FileCode, CheckCircle, Clock, GitMerge } from "lucide-react";
...
const FlowLog = ({ finding, remediation }: { finding: CodeFinding; remediation: CodeRemediation | null }) => {
  if (!remediation?.success || !remediation.githubPr) {
    return <p className="text-muted-foreground">Could not generate remediation flow.</p>;
  }

  const steps = [
    { icon: CheckCircle, name: 'Finding Received', details: `Finding ${finding.findingId} for ${finding.filePath} ingested.`, status: 'Complete' },
    { icon: CheckCircle, name: 'Analysis & Fix Generation', details: 'Automated fix generated by Code Remediation Agent.', status: 'Complete' },
    { icon: CheckCircle, name: 'GitHub PR Created', details: `PR opened: "${remediation.githubPr.title}"`, status: 'Complete' },
    { icon: Clock, name: 'Awaiting Approval', details: 'Awaiting manual review and approval before merging.', status: 'In Progress' },
    { icon: GitMerge, name: 'Merge & Close', details: 'PR will be merged automatically after approval.', status: 'Pending' }
  ];
    ...
    <div className="space-y-4">
      {steps.map((step, index) => (
        <div key={index} className="flex items-start gap-4">
          <step.icon className={cn("h-5 w-5 mt-1 flex-shrink-0", statusColors[step.status])} />
          <div>
            <p className="font-medium">{step.name} <span className={cn("text-sm", statusColors[step.status])}>({step.status})</span></p>
        ...
        {remediation && selectedFinding && (
          <Tabs defaultValue="explanation">
            <TabsList className="grid w-full grid-cols-5">
              ...
              <TabsTrigger value="json">JSON Status</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="flow" className="mt-4">
              <FlowLog finding={selectedFinding} remediation={remediation} />
            </TabsContent>
            <TabsContent value="json" className="mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Remediation Status (JSON)</CardTitle></CardHeader>
                <CardContent>
                  <pre className="bg-muted p-4 rounded-md overflow-x-auto text-sm">
                    <code>{JSON.stringify(remediation, null, 2)}</code>
                  </pre>
                </CardContent>
              </Card>
            </TabsContent>
